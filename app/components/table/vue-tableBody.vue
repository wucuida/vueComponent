...
:class="extraTrClass"
>
   	<td v-if="(expandIconAsCell || hasCheckbox) && fixed !== 'right'"
   		key="ant-table-expand-icon-placeholder" >
   	</td>
	<td colSpan={{columns.length}} v-html="expandedRowContent">
...
export default {
	...
	computed: {
		...
		extraTrClass() {
			const cls = this.expandedRowClassName(this.model)
			return `ant-table-row-expanded-row ${cls}`
		},
		// showExtra() {
		// 	return !!(this.expandedRowContent && this.isRowExpanded)
		// }
	},
	methods: {
		checkRowExpanded(record) {
		...
		initParams() {
			...
			// console.log(this.expandedRowKeys) //'2'
			this.isRowExpanded = this.checkRowExpanded(this.model)
			// let expandedRowContent;
			if (this.expandedRowRender && this.isRowExpanded) {
				if(this.fixed !== 'right'){
					let expandedRowContent = this.expandedRowRender(this.model)
		...
		toggleExtraRow() {
			// this.$nextTick(() => {this.isRowExpanded = !this.isRowExpanded})
			this.isRowExpanded = !this.isRowExpanded;
			if(!this.expandedRowContent && this.expandedRowRender){
				this.expandedRowContent = this.expandedRowRender(this.model) || '-'
			}
